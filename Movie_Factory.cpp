...
Movie* Movie_Factory::create_movie(Id_Counter_Manager& new_id_manager, const char* genre) 
{
    const char* title = User_Input::enter_text("Enter name: ", Constants::MAX_MOVIE_NAME_LENGTH);
    ...
    const char* date = User_Input::enter_date("Enter date (YYYY-MM-DD): ");
    const char* start = User_Input::enter_time("Enter start time (HH:MM): ");
    const char* finish = User_Input::enter_time("Enter finish time (HH:MM): ");
    ...
    else if (strcmp(genre, "Documentary") == 0) {
        const char* theme = User_Input::enter_text("Enter theme (nature/history/person...): ", Constants::MAX_MOVIE_THEME_LENGTH);
